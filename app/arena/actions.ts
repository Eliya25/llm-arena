...
export async function createTurn(input: {
  ...
  prompt: string;
  modelIds: string[];
  // Generated by the client for this turn and sent to /api/chat too, so the
  // route can find these message rows when its stream ends without this write
  // having to finish first (docs/scope.md Feature 12).
  clientKey: string;
...
// completeMessage/failMessage used to live here. They took the answer text and
// the metrics straight from the browser and only checked who owned the row, so
// a signed-in user could publish arbitrary text at a public thread URL under a
// model's name and feed invented speed numbers to the global leaderboard.
// Writing an answer is now the job of app/api/chat/route.ts, which is the only
// place that actually sees the model's output (docs/scope.md Feature 12).
